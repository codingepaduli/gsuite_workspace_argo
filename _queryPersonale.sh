...
function query::getDipartimentiRaggruppatiAll {
  local FIELDS="dipartimento, materie"
  local ORDERING="dipartimento"
  echo "
    SELECT ${1:-$FIELDS}
    FROM (
      SELECT 'DIPARTIMENTO_INCLUSIONE' AS dipartimento, ' ''SOSTEGNO'' ' AS materie
        UNION ALL 
      SELECT 'DIPARTIMENTO_MATEMATICA', ' ''MATEMATICA'' '
        UNION ALL 
      SELECT 'DIPARTIMENTO_UMANISTICO', ' ''LETTERE'', ''DIRITTO'' ' 
        UNION ALL 
      SELECT 'DIPARTIMENTO_INGLESE', ' ''INGLESE'' '
        UNION ALL 
      SELECT 'DIPARTIMENTO_TELEC_ELETT_INFOR', ' ''INFORMATICA'', ''ELETTRONICA'' '
        UNION ALL 
      SELECT 'DIPARTIMENTO_SCIENTIFICO', ' ''SCIENZE_MOTORIE'', ''SCIENZE'' '
        UNION ALL 
      SELECT 'DIPARTIMENTO_ODONTOTECNICO', ' ''ODONTOTECNICO'' '
        UNION ALL 
      SELECT 'DIPARTIMENTO_MACCANICA_AEREONAUTICA', ' ''DISEGNO'', ''MACCANICA_AEREONAUTICA'' '
        UNION ALL 
