        ...
        case $choice in
            ...
            6)
              ...
              echo "Prepara EMAIL degli account studenti, da inviare ai coordinatori"
                ...
                done < <($SQLITE_CMD -csv studenti.db "SELECT sz.sezione_gsuite, sz.email_coordinatore FROM $TABELLA_SEZIONI sz WHERE 1=1 $SQL_FILTRO_ANNI AND $SQL_FILTRO_SEZIONI AND sz.email_coordinatore IS NOT NULL AND LOWER(sz.email_coordinatore) != '' ORDER BY sz.sezione_gsuite" | sed 's/"//g' )
            ;;
            20)
                echo "Arrivederci!"
                exit 0
